...
function GameSummary:draw()
    ...
    if instanceOf(Spike, obj) then
        obj:draw({x = upscale_x(x), y = upscale_y(self.y)}, LIGHT)
    else
        obj:draw({x = upscale_x(x), y = upscale_y(self.y)})
    end
...
function GameSummary:draw_projectile_stats(y)
    self.y = self.y + 2
